...
def get_dashscope_api_key():
    """读取 DashScope API Key，兼容多种环境变量命名"""
    candidates = [
        "DASHSCOPE_API_KEY",
        "dashscope_api_key",
        "DASHSCOPE-API-KEY",
...
def is_valid_api_key(api_key):
    """判断 API Key 是否为有效值（排除占位符）"""
    if not api_key:
        return False
    placeholders = ("你的密钥", "你的真实密钥", "your_api_key", "sk-xxxxxxxxxxxxxxxx")
...
if not DASHSCOPE_API_KEY:
    raise ValueError(
        "错误：请设置环境变量 DASHSCOPE_API_KEY。"
        "Mac 系统里若配置的是 dashscope-api-key，请改为 DASHSCOPE_API_KEY，"
        "或在启动服务的同一终端执行 export DASHSCOPE_API_KEY=sk-你的密钥"
    )
...
def search_results(query, index, metadata):
    """检索并返回结果列表（供 API 使用，不打印日志）"""
    query_vec = np.array([get_text_embedding(query)]).astype('float32')
    distances, indices = index.search(query_vec, index.ntotal)
...
def rag_ask_structured(query, index, metadata, k=3):
    """RAG问答，返回结构化结果（供前端 API 使用）"""
    results = search_results(query, index, metadata)
...
"""
    ...
    if matched_image:
        image_info = {
            ...
            "similarity": matched_image["similarity"]
        }
    ...
    if matched_video:
        video_info = {
            ...
            "similarity": matched_video["similarity"]
        }
    ...
    return {
        ...
        "image": image_info,
        "video": video_info,
        "intent": {"want_image": want_image, "want_video": want_video}
    }
